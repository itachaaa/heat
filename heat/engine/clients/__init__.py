...
def has_client(name):
    ...
    # mgr.names() 返回所有插件的名字. 这段代码首先判断 _mgr 是否为空，
    # 然后判断传进来的插件名称否在 names 里面
    # 也就是判断 name 的插件是否加载了
...
def initialise():
    ...
    def client_is_available(client_plugin):
        if not hasattr(client_plugin.plugin, 'is_available'):
            ...
            return True    # 是否可用 如果没有参数则默认总是可用的
        # let the client plugin decide if it wants to register or not
        return client_plugin.plugin.is_available()    # 如果有这个参数，则根据这个参数的返回值来确定
